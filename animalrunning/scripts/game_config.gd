...
# ---------------------------------------------------------------- 视角（D8）
## 第一人称：相机贴在动物头部，跑动颠簸沿用节奏；死亡时自动切回第三人称看倒地动画。
const FP_EYE_HEIGHT := 1.45     # 眼睛离地高度
const FP_LOOK_AHEAD := 26.0     # 视线落点的前方距离
const FP_FOV := 74.0            # 第一人称视场角（比第三人称宽，速度感更强）
const FP_BOB := 0.07            # 跑动颠簸幅度
const FP_CAM_OFFSET_Z := 0.30   # 相机在眼睛位置略向前伸，避免贴脸穿模
